...
test-e2e: setup-test-e2e manifests generate fmt vet ## Run the e2e tests. Expected an isolated environment using Kind.
	...
	KIND=$(KIND) KIND_CLUSTER=$(KIND_CLUSTER) KUBECONFIG="$$kubeconfig" go test -tags=e2e ./test/e2e/ -v -ginkgo.v
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name k8s-operator-hermes-agent-builder
	$(CONTAINER_TOOL) buildx use k8s-operator-hermes-agent-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm k8s-operator-hermes-agent-builder
	rm Dockerfile.cross
...
.PHONY: build-crd-bundle
build-crd-bundle: manifests ## Copy the release CRD bundle for explicit Helm upgrades.
	mkdir -p dist
	cp config/crd/bases/hermes.nous.ai_hermesagents.yaml dist/hermesagents.hermes.nous.ai-crd.yaml
